    ...
    while m < loop_condition:
        ...
        for tree_update in area_limit_forest_iniPG:
            ...
            if len(fea_list):
                ...
                acc.append(train_knn(data_sample, trainY, data_predict, predictY, 3))  # 每棵树的准确率存在acc中
...
def ADAFSFOA(area_limit_forest_iniPG):
    ...
    accuracy, DR, optimal_subset = OptimalResult(trainX, trainY, predictX, predictY, resultList, feature, 'KNN', 3)
    # accuracy = max(accuracy_max)
    # DR = 1 - (1.0 * optimal_feature_subset.count(1) / num_fea_original)
    end = time.clock()
...
if __name__ == '__main__':
    ...
    inputDict1 = {'a': ['srbct', [37, 1, 10]], 'arcene': ['arcene', [1, 1, 1]]}
    inputDict2 = {'segmentation': ['segmentation', [1, 1, 10]]}
    ctime = 0
    while ctime < 10:
        ctime += 1
        for key in inputDict2:
            ...
            for loop in xrange(loop0):
                ...
                util.print_to_file('ADAFSFOA', '3NN', dataSet[0], labName, ADAFSFOA_accuracy_mean * 100, ADAFSFOA_DR_mean * 100)
