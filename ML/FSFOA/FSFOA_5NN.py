    ...
    while m < loop_condition:
        ...
        for tree_update in area_limit_forest_iniPG:
            ...
            if len(fea_list):
                ...
                acc.append(train_knn(data_sample, trainY, data_predict, predictY, 5))  # 每棵树的准确率存在acc中
...
def ADAFSFOA(area_limit_forest_iniPG):
    ...
    accuracy, DR, optimal_subset = OptimalResult(trainX, trainY, predictX, predictY, resultList, feature, 'KNN', 5)
    # accuracy = max(accuracy_max)
    # DR = 1 - (1.0 * optimal_feature_subset.count(1) / num_fea_original)
    end = time.clock()
...
if __name__ == '__main__':
    ...
    inputDict2 = {'sonar': ['sonar', [37, 1, 10]]}
    ctime = 0
    while ctime < 100:
        ctime += 1
        for key in inputDict2:
            ...
            for loop in xrange(loop0):
                ...
                util.print_to_file('ADAFSFOA', '5NN', dataSet[0], labName, ADAFSFOA_accuracy_mean * 100, ADAFSFOA_DR_mean * 100)
