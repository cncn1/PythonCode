...
def ADAFSFOA(area_limit_forest_iniPG):
    ...
    accuracy, DR, optimal_subset = OptimalResult(trainX, trainY, predictX, predictY, resultList, feature, 'KNN', 1)
    # accuracy = max(accuracy_max)
    # DR = 1 - (1.0 * optimal_feature_subset.count(1) / num_fea_original)
    end = time.clock()
...
if __name__ == '__main__':
    ...
    inputDict2 = {'srbct': ['srbct', [37, 1, 10]], 'arcene': ['arcene', [1, 1, 1]]}
    ctime = 0
    while ctime < 100:
        ctime += 1
        for key in inputDict2:
            ...
            for loop in xrange(loop0):
                ...
                util.print_to_file('ADAFSFOA', '1NN', dataSet[0], labName, ADAFSFOA_accuracy_mean * 100, ADAFSFOA_DR_mean * 100)
