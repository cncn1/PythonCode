...
for i in digits.data:
    data.append(hog(np.array(i.reshape((8, 8))), pixels_per_cell=(2, 2)))
data = scale(data)
...
def main():
    ...
    bench(AgglomerativeClustering(linkage='average', n_clusters=n_digits),
          ...
          data=data)
    pca = PCA(n_components=n_digits).fit(data)
    # data1=PCA(n_components=200).fit_transform(data)
    bb.modifiedbackbone(data, 'k-means++', "ModifiedKMeans++backbone", 2, n_digits, labels)
    bb.backbone(data, pca.components_, "KMeansPCAbackbone", 2, n_digits, labels)
    bb.backbone(data, 'k-means++', "KMeans++backbone", 2, n_digits, labels)
